...
ESP8266WiFiMyMulti::ESP8266WiFiMyMulti() {
}
...
bool ESP8266WiFiMyMulti::addAP(const char* ssid, const char *passphrase) {
...
wl_status_t ESP8266WiFiMyMulti::run(void) {
  ...
  if (status == WL_DISCONNECTED || status == WL_NO_SSID_AVAIL || status == WL_IDLE_STATUS || status == WL_CONNECT_FAILED) {
    ...
    if (scanResult > 0) {
      // scan done, analyze
      MyWifiAPEntry *bestNetwork;// {NULL,NULL,INT_MAX};
      int bestNetworkDb = INT_MIN;
      int leastTries = INT_MAX;;
      uint8 bestBSSID[6];
      int32_t bestChannel;
      ...
      for (int8_t i = 0; i < scanResult; ++i) {
        ...
        for (auto &entry:APlist) {
          if (ssid_scan == entry.ssid) { // SSID match
            ...
            if (leastTries > entry.failedConnects || (rssi_scan > bestNetworkDb && leastTries == entry.failedConnects)) { // best network
              if (sec_scan == ENC_TYPE_NONE || entry.passphrase) { // check for passphrase if not open wlan
                bestNetworkDb = rssi_scan;
                bestChannel = chan_scan;
                leastTries = entry.failedConnects;
                bestNetwork = &entry;
        ...
        DEBUG_WIFI_MULTI(" %d: [%d][%02X:%02X:%02X:%02X:%02X:%02X] %s (%d) %c (%d)\n", i, chan_scan, BSSID_scan[0], BSSID_scan[1], BSSID_scan[2], BSSID_scan[3], BSSID_scan[4], BSSID_scan[5], ssid_scan.c_str(), rssi_scan, (sec_scan == ENC_TYPE_NONE) ? ' ' : '*', tries);
        delay(0);
      }
...
bool ESP8266WiFiMyMulti::APlistAdd(const char* ssid, const char *passphrase) {
  ...
  newAP.failedConnects = 0;
  APlist.push_back(newAP);
  DEBUG_WIFI_MULTI("[WIFI][APlistAdd] add SSID: %s\n", newAP.ssid);
  return true;
}

void ESP8266WiFiMyMulti::APlistClean(void) {
  for (auto entry : APlist) {
    if (entry.ssid) {
      free(entry.ssid);
    }
